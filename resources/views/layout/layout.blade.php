...
<head>
  ...
  <script src="{{ asset('admin/jquery-3.6.1.min.js') }}"></script>
  <script src="{{ asset('admin/sweetalert2.all.min.js') }}"></script>
  <link rel="stylesheet" href="{{ asset('admin/select2.min.css') }}">
...
<style>
  ...
  .box-list-nof-rating::-webkit-scrollbar {
    width: 8px;
    background: #FFF2E7;
    border-radius: 0 8px 8px 0;
  }

  .box-list-nof-rating::-webkit-scrollbar-thumb {
    background: #FF7506;
    border-radius: 0 8px 8px 0;
  }
...
<body>
  ...
  <div id="waiting" class="hide">
    <img src="{{ asset('images/nice1.gif') }}" alt="">
  </div>
  <aside class="sidebar-nav-wrapper">
    ...
    <nav class="sidebar-nav">
      <ul>
        ...
        <li class="nav-item nav-item-has-children">
          <a href="#0" class="collapsed" data-bs-toggle="collapse" data-bs-target="#ddmenu_p" aria-controls="ddmenu_p" aria-expanded="false" aria-label="Toggle navigation">
            <span class="icon">
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <ul id="ddmenu_p" class="collapse dropdown-nav">
            <li>
              <a href="/posts"> Bài viết </a>
            </li>
            <li>
              <a href="/categories-posts"> Chuyên mục </a>
            </li>
          </ul>
        </li>
        <li class="nav-item nav-item-has-children">
          ...
          <ul id="ddmenu_3" class="collapse dropdown-nav">
            ...
            <li>
              <a href="/cate"> Loại sản phẩm </a>
            </li>
            <li>
              <a href="/size">Size sản phẩm </a>
            </li>
            <li>
        ...
        <li class="nav-item nav-item-has-children">
          <a href="#0" class="collapsed" data-bs-toggle="collapse" data-bs-target="#ddmenu_4" aria-controls="ddmenu_4" aria-expanded="false" aria-label="Toggle navigation">
            ...
            <span class="text">Giao diện</span>
          </a>
          <ul id="ddmenu_4" class="collapse dropdown-nav">
            <li>
              <a href="/sliders"> Quản lý slider </a>
            </li>
            <li>
              <a href="/infoShop"> Thông tin shop </a>
            </li>
          </ul>
        </li>
        <li class="nav-item nav-item-has-children">
          ...
          <ul id="ddmenu_55" class="collapse dropdown-nav">
            <li>
              <a href="/bills"> Tất cả hóa đơn </a>
            </li>
            <li>
              <a href="mdi-icons.html"></a>
            </li>
          </ul>
        </li>
        {{-- <li class="nav-item nav-item-has-children">
          ...
          <div class="col-lg-7 col-md-7 col-6">
            <div class="header-right">
              ...
              <div class="notification-box ml-15 d-none d-md-flex">
                <button class="dropdown-toggle notification-rating" type="button" id="notification" data-bs-toggle="dropdown" aria-expanded="false">
                  <i class="	far fa-star"></i>
                  <span></span>
                </button>
                <ul class="shadow dropdown-menu dropdown-menu-end box-list-nof-rating" aria-labelledby="notification">
  ...
  <script>
    ...
    $(document).ready(function() {
      $(window).scroll(function() {
        if (($(this).scrollTop() > 80)) {
          ...
          $(".sidebar-nav-wrapper").addClass("sidebar-scroll");
        } else {
          $(".header").removeClass("fixed-top shadow");
          $(".sidebar-nav-wrapper").removeClass("sidebar-scroll");
        }
      })
    })
    // ###############
    ...
    $(".notification-rating > span").hide()

    function loadNotificationRating() {
      AjaxSetup()
      $.ajax({
        url: "http://127.0.0.1:3000/api/loadRealTimeRatingNof",
        type: "get",
        success: function(response) {
          if (response.count > 0) {
            $(".notification-rating > span").show()
            if (response.count > 9) {
              response.count = "9+";
            }
            $(".notification-rating > span").html(response.count);
            $(".box-list-nof-rating").html(response.result);

            $(".li-each-nof-rating-realtime").each(function(index, value) {
              $(".main_stars_" + $(value).data("id")).each(function(i, v) {
            ...
            $(".notification-rating > span").hide()
            $(".box-list-nof-rating").html("Không có bài đánh giá nào cả !");
          }
        }
      })
    }
    loadNotificationRating()
    var pusher = new Pusher('d64673b10884e3d30bfd', {
      cluster: 'ap1'
    });

    var channel = pusher.subscribe('NotificationsRating');
    channel.bind('ChannelNotRating', function(data) {
      console.log(data);
      loadNotificationRating()
    });
  </script>
